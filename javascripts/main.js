...
function removeNode(nid){
	$("#loading").show();
	deleteNodeRequest(nid, function(json){
        msg = JSON.parse(json);
        if(msg.status == '200'){
...
function removeEdge(eid){
	$("#loading").show();
	deleteEdgeRequest(eid, function(json){
        msg = JSON.parse(json);
        if(msg.status == '200'){
...
function removeAllEdges(nid){
	$("#loading").show();
	deleteAllEdgesRequest(nid, function(json){
        msg = JSON.parse(json);
        if(msg.status == '200'){
...
function addNode(e, title){
	...
	$("#loading").show();
	newNodeRequest(node, function(json){
        msg = JSON.parse(json);
        if(msg.status == '200'){
...
function doSelectNode(e){
	...
	if(!source)	{
		...
		source = null;
		$("#loading").show();
		newEdgeRequest(edge, function(json){
            msg = JSON.parse(json);
			if(msg.status == '200'){
